...
def test_project_create_not_json():
    ...
    response = requests.post(url('project'), data='% not JSON', headers=apikey)
    assert response.status_code == 400, response

def test_project_create():
    "Create a project."
    data = dict(projectid=PROJECTID, name='P.Kraulis_13_01')
    response = requests.post(url('project'),
...
def test_no_such_project():
    ...
    response = requests.get(url('project', 'no such project'), headers=apikey)
    assert response.status_code == 404, response

def test_projects_list():
    "Get the list of projects."
    response = requests.get(url('projects'), headers=apikey)
...
def test_project_modify():
    ...
    response = requests.get(url('project', PROJECTID), headers=apikey)
    assert response.status_code == 200, response
    olddata = response.json()
    data = dict(name=(olddata.get('name') or 'blah') + 'xyz_123')
    response = requests.put(url('project', PROJECTID),
                            data=json.dumps(data),
                            headers=apikey)
...
def test_project_delete():
    "Delete a project."
    response = requests.delete(url('project', PROJECTID), headers=apikey)
    assert response.status_code == 204, response
